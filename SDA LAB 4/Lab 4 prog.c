...
void selection_sort_prog(int n , struct stud_rec p[],int length, char spec[]){
    struct stud_rec temp;
        for(int k = 0  ; k  < n ; k++) {
         ...
         printf("\n\nNotele aranjate in ordine descendenta, la Programare, utilizand Selection Sort : \n");
        for(int k = 0; k < n; k++) {
                if(strcmp(spec,p[k].specialitate) == 0) {
...
void selection_sort_filoz(int n , struct stud_rec p[],int length, char spec[]){
    ...
    for(int k = 0 ; k < n ;k++) {
        if(strcmp(spec,p[k].specialitate) == 0){
         ...
         printf("\n\nNotele aranjate in ordine descendenta, la Filozofie, utilizand Selection Sort : \n");
        for(int k = 0; k < n; k++) {
                if(strcmp(spec,p[k].specialitate) == 0) {
    ...
    struct stud_rec temp;
      for(int k = 0 ; k < n ; k++) {
        if(strcmp(spec,p[k].specialitate) == 0) {
         ...
         printf("\n\nNotele aranjate in ordine descendenta, la Limba Engleza, utilizand Selection Sort : \n");
        for(int k = 0; k < n; k++) {
                if(strcmp(spec,p[k].specialitate) == 0) {
                     printf("%s\t\t%d\n" ,p[k].prenume,p[k].eng);
                }
...
int main() {
    ...
    if(checker != 1) {
        ...
        printf("\n");
        calc(n, student,spec);
        Bsort_prog(student,n,spec);
        Bsort_filoz(student,n,spec);
        Bsort_eng(student,n,spec);

        printf("\n\n------------------------------------------------------------------------------\n");

        selection_sort_prog(n,student,n,spec);
        selection_sort_filoz(n,student,n,spec);
        selection_sort_eng(n,student,n,spec);
